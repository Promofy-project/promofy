 ...
 *
 * Os OITO restantes são BRIDGE VISUAL DO MARCO 2: os segmentos novos
 * nunca existiram no legado, e não há especificação de cor para eles em
 * lugar nenhum do repo (procurado em `docs/`, `tailwind.config.ts` e
 * `globals.css`). São coerentes com a família visual existente — mesma
 * estrutura de duas paradas, hexadecimais da mesma paleta — e
 * mutuamente distintos, mas são um ponto de partida honesto, não uma
 * decisão de design fechada. Refinar é escopo do Marco 3; mudar aqui não
 * exige migration, que é justamente o motivo de o banco guardar token.
 *
...
export const TEMA_CORES: Readonly<Record<string, ParDeCores>> = {
  ...
  // --- bridge visual do Marco 2 (refinável no Marco 3) ---
  grafite: { de: "#3F3F46", para: "#18181B" }, // Automotivo — quase-preto neutro
  roxo: { de: "#9333EA", para: "#6366F1" }, // Entretenimento — púrpura -> índigo
  ciano: { de: "#22D3EE", para: "#0E7490" }, // Turismo e Hotelaria — ciano -> petróleo
  violeta: { de: "#7C3AED", para: "#E879F9" }, // Moda — violeta -> fúcsia claro
  cinza: { de: "#94A3B8", para: "#475569" }, // Serviços — aço frio
  vermelho: { de: "#EF4444", para: "#B91C1C" }, // Saúde — vermelho profundo
  terra: { de: "#B45309", para: "#78350F" }, // Casa e Decoração — terroso
  amarelo: { de: "#FDE047", para: "#EAB308" }, // Infantil e Maternidade — amarelo claro
};
